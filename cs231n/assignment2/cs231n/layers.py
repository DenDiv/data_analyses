...
def conv_forward_naive(x, w, b, conv_param):
    ...
    HH = w.shape[2]
    WW = w.shape[3]
    H_htr = 1 + (H + 2 * pad - HH) // stride
    W_htr = 1 + (W + 2 * pad - WW) // stride
    out = np.zeros((N, F, H_htr, W_htr))
    ...
    x_new = np.zeros((N, C, 2*pad + H, 2*pad + W))
    x_new[:, :, pad:x_new.shape[2]-pad, pad:x_new.shape[3]-pad] = x
    for i in range(N):
        for j in range(F):

            activ_map = np.zeros((H_htr, W_htr))
            op_point = [0,0]
            for first_c in range(H_htr):
                for second_c in range(W_htr):
                    
                    activ_map[first_c, second_c] = np.sum(x_new[i, :, op_point[0]:op_point[0]+HH, op_point[1]:op_point[1]+WW]*w[j]) + b[j]
                    op_point[1] += stride
                ...
                for second_c in range(W_htr):
                    for k in range(w.shape[2]):
                        for m in range(w.shape[3]):
                            ...
                            dw[j, :, k, m] += dout[i, j, first_c, second_c] * x_new[i, :, first_c*stride + k, second_c*stride + m]
                            dx_new[i, :, first_c*stride + k, second_c*stride + m] += dout[i, j, first_c, second_c] * w[j, :, k, m]
...
def max_pool_forward_naive(x, pool_param):
    ...
    H_htr = 1 + (H - pool_height) // stride
    W_htr = 1 + (W - pool_width) // stride
    out = np.zeros((N, C, H_htr, W_htr))

    op_point = [0,0]
    for i in range(H_htr):
        for j in range(W_htr):
            
            #print(x[:, :, op_point[0]:op_point[0] + pool_height,op_point[1]:op_point[1] + pool_width])
            out[:,:,i, j] = np.amax(x[:, :, op_point[0]:op_point[0] + pool_height,op_point[1]:op_point[1] + pool_width], axis = (-1,-2))
            op_point[1] += stride
